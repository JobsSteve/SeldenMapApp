...
//
/*
 Edited by Corey Zanotti 8/12-12/12
 Changes made: 11/30:
 Figuring out a way to make the drawer have three different locations to open and close to. Formerly the code allowed the drawer to move to the right side of the sreen and to the left side of the screen. Theoretically, if I reinstate that situation, and change the right side to be the normal location and the normal location to be the left reveal location and the left reveal location to be the completely off scren location, the drawer should be working as we'd like it to be.
 */
...
@property (nonatomic, unsafe_unretained) BOOL underRightShowingCompletely;
...
//ALWAYS PRESENT DON'T NEED TO WORRY ABOUT
- (void)underRightWillAppear;
- (void)topDidReset;
//BOTH VIEWS HAVE FOCUS- BIGGEST CHANGE BETWEEN IMPLEMENTATIONS
...
@synthesize underRightShowingCompletely = _underRightShowingCompletely;
@synthesize topViewIsOffScreen = _topViewIsOffScreen;


/*
 Setting up the View Controllers and Width Layouts
 */
- (void)setTopViewController:(UIViewController *)theTopViewController
{
    ...
    [_topViewController.view setFrame:CGRectMake((0-self.view.bounds.size.width + anchorLeftRevealAmount), 0, self.view.bounds.size.width, self.view.bounds.size.width)];
...
- (void)viewDidLoad
{
    ...
    self.resetTapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(sendTapGesture:)];
    _panGesture          = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(updateTopViewHorizontalCenterWithRecognizer:)];
    
    //update pan gestures ability to pass a gesture
    //DON'T NEED WILL ALWAYS INCLUDE BOTH
    ...
    _underRightShowingCompletely = YES;
}
...
/*
 Code to deal with Tap Gestures
 */
-(void)sendTapGesture:(UITapGestureRecognizer *)recognizer
{
    if(_underRightShowing && _underRightShowingCompletely)
    {
        if([self verifyGesture:recognizer])
            [self anchorTopViewTo:ECPreview];
    } else if (_underRightShowing)
    {
        //[self setTopViewToMiddle]
        if([self verifyGesture:recognizer])
            [self resetTopView];
    }else {
        //verify tap gesture's location
        if([self verifyGesture:recognizer])
...
{
    CGPoint gestLocation = [recognizer locationInView:[self topView]];
    if (gestLocation.x >= 768 - anchorLeftPeekAmount)
    {
        return true;
    }
    return false;
...
//Code to update top view!!!! make view stay!
/*
 11-30-12 Changes to make... Need to write code for additional left state...
 Changes: on final else if change to test if the under right is partially showing or not showing at all.
 */
- (void)updateTopViewHorizontalCenterWithRecognizer:(UIPanGestureRecognizer *)recognizer
{
    ...
    //gesture began: move top view... NEEDS TO CHANGE
    /*
     Changes to make: During UIGesture began,
     1. test the top views state (is it open or closed)
     2. if its open test for the initialTouchPosition, if its less than 768 - leftPeekAmount don't do anything. if it is greater, accept the pan.
     3. if its closed test the initialTouchPosition, currentcodeworks fine.
     */
    if (recognizer.state == UIGestureRecognizerStateBegan) {
        bool rightViewShowing = [self underRightShowing];
        NSLog(rightViewShowing? @"Right View is showing":@"right view isn't showing");
        ...
        //gesture ended, get velocity and test it
    } else if (recognizer.state == UIGestureRecognizerStateEnded || recognizer.state == UIGestureRecognizerStateCancelled) {
        CGPoint currentVelocityPoint = [recognizer velocityInView:self.view];
        CGFloat currentVelocityX     = currentVelocityPoint.x;
        
        
        //code determining whether to auto complete the animation
        //new algorithm 11-30-12
        /*
         change this test to accomodate new ECLocation
         */
        if ([self underRightShowingCompletely] && currentVelocityX < 100)
        {
            [self anchorTopViewTo:ECLeft];
        }
...
- (void)anchorTopViewTo:(ECSide)side animations:(void (^)())animations onComplete:(void (^)())complete
{
    ...
    if (side == ECLeft) {
        ...
        [self setUnderRightShowingCompletely:YES];
    }
    else if (side == ECPreview)
    {
        newCenter = self.previewTopViewCenter;
        [self setUnderRightShowingCompletely:NO];
    }
    ...
    [UIView animateWithDuration:0.25f animations:^{
        ...
        if (_resetStrategy & ECPanning) {
            ...
            //potentially we could reenable tapping here?
        } else {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            NSString *key = (side == ECLeft) ? ECSlidingViewTopDidAnchorLeft : nil;//updated by CZ, 10-8-12
...
/*
 ...
 [self addTopViewSnapshot];
 dispatch_async(dispatch_get_main_queue(), ^{
 NSString *key = (side == ECLeft) ? ECSlidingViewTopDidAnchorLeft : ECSlidingViewTopDidAnchorLeft;
 [[NSNotificationCenter defaultCenter] postNotificationName:key object:self userInfo:nil];
 });
 }];
 }*/
...
//this method needs to change to test the left view's change in center location
- (void)topViewHorizontalCenterWillChange:(CGFloat)newHorizontalCenter
{
    CGPoint center = self.topView.center;
...
//again, adjust for left view reseting
- (void)topViewHorizontalCenterDidChange:(CGFloat)newHorizontalCenter
{
...
//get rid of this shit all together
- (void)addTopViewSnapshot
{
...
-(CGFloat)previewTopViewCenter
{
    if (self.previewLeftPeekAmount)
    {
        return -self.resettedCenter + self.previewLeftPeekAmount;
    }
    else {
        return NSNotFound;
    }
...
/*
 underRightWillAppear is a function to prepare the right view for appearing
 */
- (void)underRightWillAppear
{
    dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)topDidReset
{
     ...
     */
    //self.resetTapGesture.enabled = YES;
    //two options: add and remove gesture recognizers or
    //jump to a dummy function that will test underrightReveal and execute a method accordingly
    //[[self topView] addGestureRecognizer:self.resetTapGesture];
    [self setResetStrategy:(ECTapping | ECPanning)];
    //end code I've added
...
- (BOOL)topViewHasFocus
{
    return !_underRightShowing && !_topViewIsOffScreen;
}
/*
 - (void)updateUnderLeftLayout
 {
